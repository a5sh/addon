...
// NEW: Browserless.io integration for JS execution
async function renderWithBrowserless(url, env, logs) {
  const browserlessKey = env?.BROWSERLESS_KEY;
  
  if (!browserlessKey) {
    logs.push(`⚠️  No BROWSERLESS_KEY configured, skipping JS execution`);
    return null;
  }

  try {
    logs.push(`[Browserless] Rendering with JavaScript: ${url.substring(0, 60)}...`);
    ...
    const response = await fetch('https://chrome.browserless.io/content?token=' + browserlessKey, {
      ...
      body: JSON.stringify({
        url,
        waitFor: 3000, // Wait 3 seconds for JS to complete
        timeout: 15000,
      }),
    });

    if (!response.ok) {
      logs.push(`⚠️  Browserless returned ${response.status}`);
      return null;
    }

    const html = await response.text();
    logs.push(`[Browserless] ✓ Rendered successfully (${html.length} bytes)`);
    return html;
  } catch (error) {
    logs.push(`⚠️  Browserless failed: ${error.message}`);
    return null;
  }
}
...
async function resolveTechUnblockedLink(sidUrl, logs, env) {
  ...
  try {
    ...
    let goUrl = getMatch(html, /<a[^>]+id=["']two_steps_btn["'][^>]+href=["']([^"']+)["']/i)
             ...
             || getMatch(html, /<a[^>]+id=["']two_steps_btn["']/i);
    ...
    if (!goUrl || goUrl === '' || html.includes('id="two_steps_btn"')) {
      ...
      // TRY BROWSERLESS: If URL is empty, use Browserless to render and extract
      const browserHtml = await renderWithBrowserless(verifyUrl, env, logs);
      if (browserHtml) {
        goUrl = getMatch(browserHtml, /<a[^>]+id=["']two_steps_btn["'][^>]+href=["']([^"']+)["']/i)
             || getMatch(browserHtml, /id=["']two_steps_btn["'][^>]+href=["']([^"']+)["']/i);
        
        if (goUrl) {
          logs.push(`[TechUnblocked] ✓ Found goUrl via Browserless: ${goUrl.substring(0, 60)}...`);
        }
      }
      ...
      if (!goUrl) {
        logs.push(`[TechUnblocked] ✗ Could not extract go URL even with Browserless`);
        return null;
      }
    }
    ...
    if (finalUrl.includes('unblockedgames') && !finalUrl.includes('driveseed')) {
      logs.push(`[TechUnblocked] Following JS redirect from unblockedgames...`);
      
      const goHtml = await goResp.text();
      let jsRedirect = getMatch(goHtml, /window\.location(?:\.replace|\.href)?\s*(?:\(\s*|=\s*)["']([^"']+)["']/i)
                      || getMatch(goHtml, /<meta[^>]+http-equiv=["']?refresh["']?[^>]+content=["']?[^"']*url=([^"']+)["']?/i);
      
      // If regex fails, try Browserless for pure JS execution
      if (!jsRedirect && env?.BROWSERLESS_KEY) {
        logs.push(`[TechUnblocked] Regex failed, trying Browserless for JS evaluation...`);
        const browserResult = await renderWithBrowserless(finalUrl, env, logs);
        if (browserResult) {
          jsRedirect = getMatch(browserResult, /driveseed[^"']*/) || 
                      getMatch(browserResult, /http[^"']*driveseed[^"']*/);
        }
      }
